                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if self.config.get("ttrl", {}).get("enable", False):
                            ...
                            process_reward_strategy = self.config.ttrl.get("process_reward_strategy", "avg")
                            process_lcs_norm = self.config.ttrl.get("process_lcs_norm", "avg")
                            process_lcs_max_tokens = self.config.ttrl.get("process_lcs_max_tokens", 3000)
                            # 🆕 对比学习参数
                            use_contrastive_process_reward = self.config.ttrl.get("use_contrastive_process_reward", False)
                            contrastive_temperature = self.config.ttrl.get("contrastive_temperature", 0.1)
                            batch, gen_batch_output = apply_ttrl_gt(
                                batch, 
                                gen_batch_output, 
                                self.config.ttrl.n_votes_per_prompt, 
                                self.tokenizer, 
                                process_reward_weight, 
                                process_reward_strategy, 
                                process_lcs_norm,
